...
func main() {
	...
	hallBtnTx := make(chan elevio.ButtonEvent) // Specific to a single elevator
	...
	hallBtnRx := make(chan elevio.ButtonEvent) // For the master
	...
	_ = hallBtnRx // To avoid unused variable error
	_ = hallOrderTx
	_ = singleStateRx
	_ = AllStatesRx
	_ = AllStatesTx
	...
	var d elevio.MotorDirection = elevio.MD_Down // Needed for the initialization of the elevator

	initSingleElev(d, drv_floors) // Initializes the elevator
	consumer1drv_floors := make(chan int)
	consumer2drv_floors := make(chan int)
	go relay(drv_floors, consumer1drv_floors, consumer2drv_floors) // Relays the floor updates to two different channels

	d = elevio.MD_Stop // Stops the elevator
	...
	for { // Main loop
		...
		case a := <-hallOrderRx: // Received an HallOrderMsg from the master
			...
			fmt.Printf("Single elevator received order on hallOrderRx, its id: %v\n", id)
